  ...
  return <main>
    ...
    <section className="scene living-culture">
      ...
      <div className="artist-stack">
        <article className="artist-card artist-rgd" style={{ transform:`translateY(${Math.max(-70,Math.min(70,(scroll-5450)*-.035))}px)` }}><img src="/images/rgd-don.png" alt="РГД — современный музыкальный коллектив с Дона"/><div><span>01 / ДОН</span><h3>РГД</h3><p>Ритм земли. Голос поколения.</p></div></article>
        <article className="artist-card artist-layered artist-beloboka" style={{ transform:`translateY(${Math.max(-55,Math.min(55,(scroll-5650)*.026))}px)` }}><img className="artist-landscape" src="/images/don-steppe-parallax.png" alt="" style={{ transform:`translate3d(${Math.max(-24,Math.min(24,(scroll-5650)*-.012))}px,${Math.max(-18,Math.min(18,(scroll-5650)*.009))}px,0) scale(1.14)` }}/><img className="artist-person" src="/images/beloboka-real.png" alt="BELOBOKA — современный электронный фолк-проект" style={{ transform:`translate3d(${Math.max(-14,Math.min(14,(scroll-5650)*.008))}px,${Math.max(-42,Math.min(42,(scroll-5650)*-.022))}px,0) scale(1.03)` }}/><div><span>02 / НОВЫЙ ФОЛК</span><h3>BELOBOKA</h3><p>Фольклор, индустриальный звук и новая сцена.</p></div></article>
        <article className="artist-card artist-layered artist-yazhevika" style={{ transform:`translateY(${Math.max(-75,Math.min(75,(scroll-5850)*-.03))}px)` }}><img className="artist-landscape" src="/images/don-steppe-parallax.png" alt="" style={{ transform:`translate3d(${Math.max(-28,Math.min(28,(scroll-5850)*.014))}px,${Math.max(-20,Math.min(20,(scroll-5850)*-.01))}px,0) scale(1.16)` }}/><img className="artist-person" src="/images/yazhevika-real.webp" alt="YAZHEVIKA — донская казачка и современная исполнительница" style={{ transform:`translate3d(${Math.max(-18,Math.min(18,(scroll-5850)*-.01))}px,${Math.max(-48,Math.min(48,(scroll-5850)*.025))}px,0) scale(1.12)` }}/><div><span>03 / ГОЛОС</span><h3>YAZHEVIKA</h3><p>Сталь, крест и женская сила Дона.</p></div></article>
      </div>
