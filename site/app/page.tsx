...
export default function Home() {
  ...
  return <main>
    ...
    <style>{`.nation-bg{transition:transform .16s ease-out!important;will-change:transform}.nation-people-soft,.nation-people-layer{position:absolute;inset:-7%;width:114%;height:114%;object-fit:cover;object-position:center;pointer-events:none;will-change:transform;transition:transform .13s ease-out}.nation-people-soft{z-index:1;opacity:.82;filter:blur(8px) saturate(.92) brightness(.82);transform-origin:center bottom;mask-image:linear-gradient(to bottom,transparent 0 49%,#000 61% 96%,transparent 100%);-webkit-mask-image:linear-gradient(to bottom,transparent 0 49%,#000 61% 96%,transparent 100%)}.nation-people-layer{z-index:2!important;opacity:1!important;mix-blend-mode:normal!important;filter:saturate(.96) contrast(1.05) brightness(.91) drop-shadow(0 22px 28px #000b)!important;mask-image:linear-gradient(to bottom,transparent 0 51%,#000 59% 97%,transparent 100%)!important;-webkit-mask-image:linear-gradient(to bottom,transparent 0 51%,#000 59% 97%,transparent 100%)!important}.nation-shade{z-index:3!important}.nation-copy,.nation-emblem,.nation-word{z-index:4!important}`}</style>
    ...
    <section id="nation" className="scene nation-scene" onMouseMove={event=>{const r=event.currentTarget.getBoundingClientRect();setNationPointer({x:(event.clientX-r.left)/r.width-.5,y:(event.clientY-r.top)/r.height-.5})}} onMouseLeave={()=>setNationPointer({x:0,y:0})}>
      ...
      <img className="nation-people-soft" src="/images/nation-people-alpha-v2.png" alt="" aria-hidden="true" style={{transform:`translate3d(${nationPointer.x*16}px,${Math.max(-30,Math.min(30,(scroll-3000)*-.018))+nationPointer.y*10}px,0) scale(1.064)`}}/>
